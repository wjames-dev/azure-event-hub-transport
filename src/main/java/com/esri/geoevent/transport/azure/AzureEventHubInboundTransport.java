...
public class AzureEventHubInboundTransport extends InboundTransportBase {
  ...
  private boolean useTransportTypeAMPQ = true;
  private String storageContainerName = "";
  private boolean dataIsAvroEncoded = false;
  ...
  public void readProperties() throws Exception {
    try {
      ...
      useTransportTypeAMPQ = ((Boolean)(getProperty(AzureEventHubInboundTransportDefinition.USE_TRANSPORT_TYPE_AMPQ_PROPERTY_NAME).getValue()));
      storageContainerName =  getProperty(AzureEventHubInboundTransportDefinition.STORAGE_CONTAINER_NAME_PROPERTY_NAME).getValueAsString();
      dataIsAvroEncoded = ((Boolean)(getProperty(AzureEventHubInboundTransportDefinition.DATA_IS_AVRO_ENCODED_PROPERTY_NAME).getValue()));
    } catch (Exception e) {
  ...
  private void createNewHost() {
    try {
      ...
      if (!provideEventHubConnectionString) {
        ...
        builder = new ConnectionStringBuilder() //eventHubEndpointUri, eventHubName, eventHubAccessPolicy, eventHubAccessKey);
	      .setNamespaceName(eventHubEndpoint) //.setNamespaceName("mvs-stg-ehn-tco")
		  .setEventHubName(eventHubName)
		  .setSasKeyName(eventHubAccessPolicy)
		  .setSasKey(eventHubAccessKey);
      }
      else {
    	  builder = new ConnectionStringBuilder(eventHubConnectionString);
      }
      
      if(useTransportTypeAMPQ) {
      ...
      host = new EventProcessorHost(EventProcessorHost.createHostName(null), eventHubName, eventHubConsumerGroupName, eventHubConnectionString, storageConnectionString, storageContainerName);//"mvsstgtco1");
	  ...
	  if (bytes != null && bytes.length > 0) {
	      ...
	      try {
	    	if(dataIsAvroEncoded) {
	    		...
	    		bb = ByteBuffer.allocate(newBytes.length);
	    		bb.put(newBytes);
	    	}
